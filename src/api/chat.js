...
router.post('/send', requireAuth, async (req, res) => {
  ...
  if (!apiKey) {
    return res.json({ reply: 'Configura tu OpenRouter API Key en /setup para activar el agente de contenidos.' });
  }
  ...
  try {
    ...
    res.status(500).json({ error: 'Error al contactar el agente de contenidos.' });
  }
});
